...
public class UserList : GreeterList
{
    ...
    private string _default_session = "cinnamon";
    ...
    construct
    {
        ...
        if (!CvGreeter.singleton.test_mode &&
            Cv.singleton.show_remote_login_hint ())
    ...
    private void on_remote_login_service_vanished (DBusConnection conn, string name)
    {
        ...
        if (CvGreeter.singleton.hide_users_hint ()) {
            add_manual_entry();
    ...
    private void remote_login_respond_cb ()
    {
        ...
        {
            CvGreeter.singleton.authenticate_remote (get_lightdm_session (), null);
    ...
    private void show_remote_account_dialog ()
    {
        ...
        dialog.response.connect ((id) =>
        {
            if (id == 1)
            {
                ...
                {
                    greeter_authenticating_user = selected_entry.id;
                    CvGreeter.singleton.authenticate_remote (config_session, null);
                }
            }
            dialog.destroy ();
        });
    ...
    {
        CvGreeter.singleton.set_state ("last-user", username);
    ...
    protected void prompt_box_respond_cb (string[] responses)
    {
        ...
        foreach (var response in responses)
        {
            if (CvGreeter.singleton.test_mode)
                ...
                CvGreeter.singleton.respond (response);
        }
    }
    ...
    private void prompt_box_show_options_cb ()
    {
        ...
        CvGreeter.singleton.push_list (session_chooser);
    }

    private void session_clicked_cb (string session)
    {
        this.session = session;
        CvGreeter.singleton.pop_list ();
    }
    ...
    private void fill_list ()
    {
        ...
        {
            default_session = CvGreeter.singleton.default_session_hint ();
            always_show_manual = CvGreeter.singleton.show_manual_login_hint ();
            if (!CvGreeter.singleton.hide_users_hint ())
            {
                var users = LightDM.UserList.get_instance ();
            ...
            if (CvGreeter.singleton.has_guest_account_hint ())
            {
            ...
            var last_user = CvGreeter.singleton.get_state ("last-user");
            if (CvGreeter.singleton.select_user_hint () != null)
                set_active_entry (CvGreeter.singleton.select_user_hint ());
    ...
    protected override void show_prompt_cb (string text, LightDM.PromptType type)
    {
        if (selected_entry.id.has_prefix ("*remote_login"))
        {
            if (text == "remote login:")
            {
                ...
                CvGreeter.singleton.respond (answer);
            }
    ...
    private void test_fill_list ()
    {
        ...
        offer_guest = CvGreeter.singleton.has_guest_account_hint();
        always_show_manual = CvGreeter.singleton.show_manual_login_hint();

        key_press_event.connect (test_key_press_cb);

        if (CvGreeter.singleton.show_remote_login_hint())
            Timeout.add (1000, () =>
            {
    ...
    protected override void test_start_authentication ()
    {
        ...
        case "auth-error":
            ...
            break;
        case "info-prompt":
            show_message_cb ("Welcome to CV Greeter", LightDM.MessageType.INFO);
            show_prompt_cb ("Password:", LightDM.PromptType.SECRET);
            break;
        case "long-info-prompt":
            show_message_cb ("Welcome to CV Greeter\n\nWe like to annoy you with long messages.\nLike this one\n\nThis is the last line of a multiple line message.", LightDM.MessageType.INFO);
            show_prompt_cb ("Password:", LightDM.PromptType.SECRET);
            break;
        case "wide-info-prompt":
            show_message_cb ("Welcome to CV Greeter, the greeteriest greeter that ever did appear in these fine lands", LightDM.MessageType.INFO);
            show_prompt_cb ("Password:", LightDM.PromptType.SECRET);
            break;
