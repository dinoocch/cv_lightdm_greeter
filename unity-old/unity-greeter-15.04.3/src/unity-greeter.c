/* unity-greeter.c generated by valac 0.26.2, the Vala compiler
...
static cairo_surface_t* unity_greeter_create_root_surface (GdkScreen* screen) {
	...
	_tmp8_ = gdk_x11_window_get_xid (G_TYPE_CHECK_INSTANCE_TYPE (_tmp7_, gdk_x11_window_get_type ()) ? ((GdkX11Window*) _tmp7_) : NULL);
	_tmp9_ = screen;
	...
	if (!_tmp69_) {
		{
			...
			GPid _tmp73_ = 0;
			g_shell_parse_argv ("upstart --user --startup-event indicator-services-start", &_tmp71_, &_tmp70_, &_inner_error_);
			argv = (_vala_array_free (argv, argv_length1, (GDestroyNotify) g_free), NULL);
			argv = _tmp70_;
