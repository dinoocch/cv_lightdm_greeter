/* greeter-list.c generated by valac 0.26.2, the Vala compiler
...
static void __lambda41_ (ListDBusInterface* self, const gchar* name);
static void ___lambda41__greeter_list_entry_selected (GreeterList* _sender, const gchar* name, gpointer self);
...
static void _dbus_list_dbus_interface_get_active_entry (ListDBusInterface* self, GVariant* _parameters_, GDBusMethodInvocation* invocation);
static void _dbus_list_dbus_interface_set_active_entry (ListDBusInterface* self, GVariant* _parameters_, GDBusMethodInvocation* invocation);
...
static void __greeter_list___lambda44__g_object_notify (GObject* _sender, GParamSpec* pspec, gpointer self);
...
ListDBusInterface* list_dbus_interface_construct (GType object_type, GreeterList* list) {
	...
	g_signal_connect_object (_tmp2_, "entry-selected", (GCallback) ___lambda41__greeter_list_entry_selected, self, 0);
	return self;
}
...
DashEntry* greeter_list_add_prompt (GreeterList* self, const gchar* text, gboolean secret) {
	...
	DashEntry* _tmp16_ = NULL;
	GreeterListMode _tmp17_ = 0;
	g_return_val_if_fail (self != NULL, NULL);
	...
	if (_tmp2_) {
		...
		gchar* _tmp11_ = NULL;
		_tmp7_ = _ ("Enter password for %s");
		_tmp8_ = self->priv->_selected_entry;
		_tmp9_ = prompt_box_get_label (_tmp8_);
		_tmp10_ = _tmp9_;
		_tmp11_ = g_strdup_printf (_tmp7_, _tmp10_);
		_g_free0 (accessible_text);
		accessible_text = _tmp11_;
	}
	_tmp12_ = self->priv->_selected_entry;
	_tmp13_ = text;
	_tmp14_ = accessible_text;
	_tmp15_ = secret;
	_tmp16_ = prompt_box_add_prompt (_tmp12_, _tmp13_, _tmp14_, _tmp15_);
	prompt = _tmp16_;
	_tmp17_ = self->mode;
	if (_tmp17_ != GREETER_LIST_MODE_SCROLLING) {
		PromptBox* _tmp18_ = NULL;
		_tmp18_ = self->priv->_selected_entry;
		prompt_box_show_prompts (_tmp18_);
	}
...
gint greeter_list_compare_entry (PromptBox* a, PromptBox* b) {
	...
	if (_tmp0_) {
		...
		_tmp22_ = _tmp21_;
		if (g_strcmp0 (_tmp22_, "*guest") == 0) {
			result = 1;
			return result;
		}
	}
	_tmp23_ = a;
	_tmp24_ = prompt_box_get_label (_tmp23_);
	_tmp25_ = _tmp24_;
	_tmp26_ = b;
	_tmp27_ = prompt_box_get_label (_tmp26_);
	_tmp28_ = _tmp27_;
	_tmp29_ = g_ascii_strcasecmp (_tmp25_, _tmp28_);
	result = _tmp29_;
	return result;
}
...
void greeter_list_set_active_entry (GreeterList* self, const gchar* name) {
	...
	if (_tmp2_ != NULL) {
		...
		_tmp11_ = e;
		_tmp12_ = direction;
		greeter_list_select_entry (self, _tmp11_, _tmp12_, TRUE);
	}
	_g_object_unref0 (e);
}
...
void greeter_list_remove_entry_by_entry (GreeterList* self, PromptBox* entry) {
	...
	if (_tmp6_ == _tmp7_) {
		...
		_tmp18_ = self->entries;
		_tmp19_ = index;
		_tmp20_ = g_list_nth_data (_tmp18_, (guint) _tmp19_);
		if (((PromptBox*) _tmp20_) != NULL) {
			GList* _tmp21_ = NULL;
			gint _tmp22_ = 0;
			gconstpointer _tmp23_ = NULL;
			_tmp21_ = self->entries;
			_tmp22_ = index;
			_tmp23_ = g_list_nth_data (_tmp21_, (guint) _tmp22_);
			greeter_list_select_entry (self, (PromptBox*) _tmp23_, -1.0, TRUE);
		}
	}
	_tmp25_ = greeter_list_have_entries (self);
	if (!_tmp25_) {
		UnityGreeter* _tmp26_ = NULL;
		gboolean _tmp27_ = FALSE;
		_tmp26_ = unity_greeter_singleton;
		_tmp27_ = unity_greeter_show_remote_login_hint (_tmp26_);
		_tmp24_ = !_tmp27_;
	} else {
		_tmp24_ = FALSE;
	}
...
void greeter_list_greeter_box_size_allocate_cb (GreeterList* self, GtkAllocation* allocation) {
	...
	self->priv->cached_box_height = _tmp7_;
}
		...
		for (entry_it = entry_collection; entry_it != NULL; entry_it = entry_it->next) {
			...
			entry = _tmp1_;
			{
				...
				if (_tmp4_) {
					...
					_tmp9_ = entry;
					_tmp10_ = position;
					greeter_list_move_entry (self, _tmp9_, _tmp10_);
					_tmp13_ = position;
					if (_tmp13_ == ((gdouble) 0)) {
						DashBox* _tmp14_ = NULL;
						gboolean _tmp15_ = FALSE;
						gboolean _tmp16_ = FALSE;
						_tmp14_ = self->greeter_box;
						_tmp15_ = dash_box_get_has_base (_tmp14_);
						_tmp16_ = _tmp15_;
						_tmp12_ = _tmp16_;
					} else {
						_tmp12_ = FALSE;
					...
					if (_tmp12_) {
						...
						_tmp18_ = dash_box_get_base_alpha (_tmp17_);
						_tmp19_ = _tmp18_;
						_tmp11_ = _tmp19_ == 0.0;
					} else {
						_tmp11_ = FALSE;
...
void greeter_list_select_entry (GreeterList* self, PromptBox* entry, gdouble direction, gboolean do_scroll) {
	...
	if (_tmp2_ != ((gdouble) _tmp5_)) {
		...
		if (_tmp11_) {
			...
			gint _tmp16_ = 0;
			gdouble _tmp17_ = 0.0;
			AnimateTimer* _tmp18_ = NULL;
			gdouble _tmp19_ = 0.0;
			gdouble _tmp20_ = 0.0;
			gint _tmp21_ = 0;
			_tmp15_ = new_direction;
			_tmp16_ = new_target;
			_tmp17_ = new_start;
			new_distance = _tmp15_ * (_tmp16_ - _tmp17_);
			_tmp18_ = self->priv->scroll_timer;
			_tmp19_ = new_distance;
			_tmp20_ = fabs (_tmp19_);
			_tmp21_ = MIN ((gint) (100 * _tmp20_), 500);
			animate_timer_reset (_tmp18_, 250 + _tmp21_);
		...
		_tmp20_ = gtk_container_get_children ((GtkContainer*) _tmp19_);
		{
			...
			for (child_it = child_collection; child_it != NULL; child_it = child_it->next) {
				...
				child = (GtkWidget*) child_it->data;
				{
					...
					_tmp21_ = child;
					_tmp22_ = self->greeter_box;
					if (_tmp21_ != G_TYPE_CHECK_INSTANCE_CAST (_tmp22_, gtk_widget_get_type (), GtkWidget)) {
...
static void greeter_list_real_show_prompt_cb (GreeterList* self, const gchar* text, LightDMPromptType type) {
	...
	LightDMPromptType _tmp17_ = 0;
	DashEntry* _tmp18_ = NULL;
	...
	if (_tmp0_) {
		...
		_tmp7_ = _tmp6_->test_mode;
		if (_tmp7_) {
			const gchar* _tmp8_ = NULL;
			_tmp8_ = self->test_username;
			greeter_list_set_manual_name (self, _tmp8_);
		} else {
			UnityGreeter* _tmp9_ = NULL;
			gchar* _tmp10_ = NULL;
			gchar* _tmp11_ = NULL;
			_tmp9_ = unity_greeter_singleton;
			_tmp10_ = unity_greeter_authentication_user (_tmp9_);
			_tmp11_ = _tmp10_;
			greeter_list_set_manual_name (self, _tmp11_);
			_g_free0 (_tmp11_);
		}
	}
	self->prompted = TRUE;
	...
	_tmp16_ = text;
	_tmp17_ = type;
	_tmp18_ = greeter_list_add_prompt (self, _tmp16_, _tmp17_ == LIGHTDM_PROMPT_TYPE_SECRET);
	_tmp19_ = _tmp18_;
...
static void greeter_list_real_authentication_complete_cb (GreeterList* self) {
	...
	if (_tmp9_) {
		...
		if (_tmp10_) {
			...
			_tmp14_ = _tmp13_->test_mode;
			if (_tmp14_) {
				greeter_list_start_session (self);
			} else {
				Background* _tmp15_ = NULL;
				gdouble _tmp16_ = 0.0;
				gdouble _tmp17_ = 0.0;
				_tmp15_ = self->priv->_background;
				_tmp16_ = background_get_alpha (_tmp15_);
				_tmp17_ = _tmp16_;
				if (_tmp17_ == 1.0) {
					greeter_list_start_session (self);
				} else {
					Background* _tmp18_ = NULL;
					_tmp18_ = self->priv->_background;
					g_signal_connect_object ((GObject*) _tmp18_, "notify::alpha", (GCallback) _greeter_list_background_loaded_cb_g_object_notify, self, 0);
				}
			}
		} else {
			self->prompted = TRUE;
		...
		_tmp19_ = self->prompted;
		if (_tmp19_) {
			gboolean _tmp20_ = FALSE;
			PromptBox* _tmp22_ = NULL;
			_tmp20_ = self->will_clear;
			if (_tmp20_) {
				const gchar* _tmp21_ = NULL;
				_tmp21_ = _ ("Invalid password, please try again");
				greeter_list_show_message (self, _tmp21_, TRUE);
			}
			_tmp22_ = self->priv->_selected_entry;
			prompt_box_reset_spinners (_tmp22_);
			...
			if (!_tmp25_) {
				...
				_tmp26_ = _ ("Failed to authenticate");
				greeter_list_show_message (self, _tmp26_, TRUE);
			}
...
static void greeter_list_class_init (GreeterListClass * klass) {
	...
	((GreeterListClass *) klass)->get_selected_id = greeter_list_real_get_selected_id;
	((GtkWidgetClass *) klass)->get_preferred_width = greeter_list_real_get_preferred_width;
	((GtkWidgetClass *) klass)->get_preferred_height = greeter_list_real_get_preferred_height;
	((GtkWidgetClass *) klass)->grab_focus = greeter_list_real_grab_focus;
	((GreeterListClass *) klass)->focus_prompt = greeter_list_real_focus_prompt;
	((GreeterListClass *) klass)->show_authenticated = greeter_list_real_show_authenticated;
	((GreeterListClass *) klass)->insert_entry = greeter_list_real_insert_entry;
	((GreeterListClass *) klass)->add_manual_entry = greeter_list_real_add_manual_entry;
	((GreeterListClass *) klass)->get_position_y = greeter_list_real_get_position_y;
	((GreeterListClass *) klass)->setup_prompt_box = greeter_list_real_setup_prompt_box;
	((GtkWidgetClass *) klass)->realize = greeter_list_real_realize;
	((GtkWidgetClass *) klass)->size_allocate = greeter_list_real_size_allocate;
	((GtkWidgetClass *) klass)->draw = greeter_list_real_draw;
	((GreeterListClass *) klass)->show_prompt_cb = greeter_list_real_show_prompt_cb;
	((GreeterListClass *) klass)->authentication_complete_cb = greeter_list_real_authentication_complete_cb;
	((GreeterListClass *) klass)->start_authentication = greeter_list_real_start_authentication;
	((GreeterListClass *) klass)->get_lightdm_session = greeter_list_real_get_lightdm_session;
	((GreeterListClass *) klass)->test_start_authentication = greeter_list_real_test_start_authentication;
