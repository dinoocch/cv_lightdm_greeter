/* main-window.c generated by valac 0.26.2, the Vala compiler
...
static void main_window_monitors_changed_cb (MainWindow* self, GdkScreen* screen) {
	...
	self->priv->primary_monitor = NULL;
	{
		...
		i = 0;
		{
			...
			while (TRUE) {
				...
				_tmp8_ = i;
				_tmp9_ = screen;
				_tmp10_ = gdk_screen_get_n_monitors (_tmp9_);
				if (!(_tmp8_ < _tmp10_)) {
					break;
				}
				_tmp11_ = screen;
				_tmp12_ = i;
				gdk_screen_get_monitor_geometry (_tmp11_, _tmp12_, &_tmp13_);
				geometry = _tmp13_;
				_tmp14_ = i;
				_tmp15_ = geometry;
				...
				_tmp21_ = geometry;
				_tmp22_ = _tmp21_.y;
				g_debug ("main-window.vala:193: Monitor %d is %dx%d pixels at %d,%d", _tmp14_, _tmp16_, _tmp18_, _tmp20_, _tmp22_);
				_tmp23_ = screen;
				_tmp24_ = i;
				_tmp25_ = main_window_monitor_is_unique_position (self, _tmp23_, _tmp24_);
				if (_tmp25_) {
					...
					_tmp34_ = monitor_new (_tmp27_, _tmp29_, _tmp31_, _tmp33_);
					monitor = _tmp34_;
					_tmp35_ = monitor;
					_tmp36_ = _monitor_ref0 (_tmp35_);
					self->priv->monitors = g_list_append (self->priv->monitors, _tmp36_);
					_tmp38_ = self->priv->primary_monitor;
					if (_tmp38_ == NULL) {
						_tmp37_ = TRUE;
					} else {
						gint _tmp39_ = 0;
					...
					if (_tmp37_) {
						...
						Monitor* _tmp42_ = NULL;
						_tmp41_ = monitor;
						_tmp42_ = _monitor_ref0 (_tmp41_);
						_monitor_unref0 (self->priv->primary_monitor);
						self->priv->primary_monitor = _tmp42_;
					}
	...
	_tmp44_ = self->priv->monitors;
	background_set_monitors (_tmp43_, _tmp44_);
	_tmp45_ = screen;
	_tmp46_ = gdk_screen_get_width (_tmp45_);
	_tmp47_ = screen;
	_tmp48_ = gdk_screen_get_height (_tmp47_);
	gtk_window_resize ((GtkWindow*) self, _tmp46_, _tmp48_);
	gtk_window_move ((GtkWindow*) self, 0, 0);
	_tmp49_ = self->priv->primary_monitor;
	main_window_move_to_monitor (self, _tmp49_);
}
	...
	g0 = _tmp2_;
	{
		...
		i = _tmp3_ + 1;
		{
			...
			while (TRUE) {
				...
				_tmp6_ = i;
				_tmp7_ = screen;
				_tmp8_ = gdk_screen_get_n_monitors (_tmp7_);
				if (!(_tmp6_ < _tmp8_)) {
					break;
				}
				_tmp9_ = screen;
		...
		for (m_it = m_collection; m_it != NULL; m_it = m_it->next) {
			...
			m = _tmp23_;
			{
				...
				if (_tmp25_) {
					...
					_tmp24_ = _tmp38_ <= (_tmp40_ + _tmp42_);
				} else {
					_tmp24_ = FALSE;
				}
				if (_tmp24_) {
					...
					_tmp43_ = m;
					main_window_move_to_monitor (self, _tmp43_);
					_monitor_unref0 (m);
					break;
				}
				_monitor_unref0 (m);
...
static gboolean main_window_real_key_press_event (GtkWidget* base, GdkEventKey* event) {
	...
	if (_tmp5_) {
		...
		if (!_tmp11_) {
			...
			if (_tmp15_) {
				...
				_tmp22_ = shift_mask;
				_tmp23_ = shift_mask;
				_tmp14_ = (_tmp21_ & _tmp22_) == _tmp23_;
			} else {
				_tmp14_ = FALSE;
			}
			if (_tmp14_) {
				...
				if (_tmp27_ == ((guint) GDK_KEY_Control_L)) {
					_tmp25_ = TRUE;
				} else {
					...
					_tmp25_ = _tmp29_ == ((guint) GDK_KEY_Control_R);
				}
				if (_tmp25_) {
					GdkEventKey* _tmp30_ = NULL;
					GdkModifierType _tmp31_ = 0;
					GdkModifierType _tmp32_ = 0;
					GdkModifierType _tmp33_ = 0;
					_tmp30_ = event;
					_tmp31_ = _tmp30_->state;
					_tmp32_ = control_mask;
					_tmp33_ = control_mask;
					_tmp24_ = (_tmp31_ & _tmp32_) == _tmp33_;
				} else {
					_tmp24_ = FALSE;
				}
				_tmp13_ = _tmp24_;
			...
			if (_tmp13_) {
				...
				if (_tmp37_ == ((guint) GDK_KEY_Alt_L)) {
					_tmp35_ = TRUE;
				} else {
			...
			if (_tmp12_) {
				UserList* _tmp44_ = NULL;
				g_debug ("main-window.vala:297: Hidden user key combination detected");
				_tmp44_ = user_list;
				user_list_set_show_hidden_users (_tmp44_, TRUE);
				result = TRUE;
	...
	switch (_tmp46_) {
		case GDK_KEY_Escape:
		{
			...
			ShutdownDialog* _tmp51_ = NULL;
			_tmp47_ = self->priv->login_box;
			_tmp48_ = gtk_widget_get_sensitive ((GtkWidget*) _tmp47_);
			_tmp49_ = _tmp48_;
			if (_tmp49_) {
				GreeterList* _tmp50_ = NULL;
				_tmp50_ = top;
				greeter_list_cancel_authentication (_tmp50_);
			}
			_tmp51_ = self->priv->shutdown_dialog;
			if (_tmp51_ != NULL) {
				ShutdownDialog* _tmp52_ = NULL;
				_tmp52_ = self->priv->shutdown_dialog;
				shutdown_dialog_cancel (_tmp52_);
			}
			result = TRUE;
			_g_object_unref0 (top);
			return result;
		...
		case GDK_KEY_KP_Page_Up:
		{
			...
			_tmp54_ = gtk_widget_get_sensitive ((GtkWidget*) _tmp53_);
			_tmp55_ = _tmp54_;
			if (_tmp55_) {
				GreeterList* _tmp56_ = NULL;
				_tmp56_ = top;
				greeter_list_scroll (_tmp56_, GREETER_LIST_SCROLL_TARGET_START);
			}
			result = TRUE;
			_g_object_unref0 (top);
			return result;
		...
		case GDK_KEY_KP_Page_Down:
		{
			...
			_tmp57_ = self->priv->login_box;
			_tmp58_ = gtk_widget_get_sensitive ((GtkWidget*) _tmp57_);
			_tmp59_ = _tmp58_;
			if (_tmp59_) {
				GreeterList* _tmp60_ = NULL;
				_tmp60_ = top;
				greeter_list_scroll (_tmp60_, GREETER_LIST_SCROLL_TARGET_END);
			}
			result = TRUE;
			_g_object_unref0 (top);
			return result;
		...
		case GDK_KEY_KP_Up:
		{
			...
			if (_tmp63_) {
				...
				_tmp64_ = top;
				greeter_list_scroll (_tmp64_, GREETER_LIST_SCROLL_TARGET_UP);
			}
			result = TRUE;
			_g_object_unref0 (top);
			return result;
		...
		case GDK_KEY_KP_Down:
		{
			...
			if (_tmp67_) {
				...
				_tmp68_ = top;
				greeter_list_scroll (_tmp68_, GREETER_LIST_SCROLL_TARGET_DOWN);
			}
			result = TRUE;
			_g_object_unref0 (top);
			return result;
		...
		{
			ShutdownDialog* _tmp69_ = NULL;
			_tmp69_ = self->priv->shutdown_dialog;
			if (_tmp69_ != NULL) {
				ShutdownDialog* _tmp70_ = NULL;
				_tmp70_ = self->priv->shutdown_dialog;
				shutdown_dialog_focus_prev (_tmp70_);
			}
			result = TRUE;
			_g_object_unref0 (top);
			return result;
		...
		{
			ShutdownDialog* _tmp71_ = NULL;
			_tmp71_ = self->priv->shutdown_dialog;
			if (_tmp71_ != NULL) {
				ShutdownDialog* _tmp72_ = NULL;
				_tmp72_ = self->priv->shutdown_dialog;
				shutdown_dialog_focus_next (_tmp72_);
			}
			result = TRUE;
			_g_object_unref0 (top);
			return result;
		...
		case GDK_KEY_F10:
		{
			...
			_tmp75_ = _tmp74_;
			if (_tmp75_) {
				MenuBar* _tmp76_ = NULL;
				_tmp76_ = self->menubar;
				gtk_menu_shell_select_first ((GtkMenuShell*) _tmp76_, FALSE);
			}
			result = TRUE;
			_g_object_unref0 (top);
			return result;
		...
		case GDK_KEY_z:
		{
			...
			UnityGreeter* _tmp78_ = NULL;
			gboolean _tmp79_ = FALSE;
			_tmp78_ = unity_greeter_singleton;
			_tmp79_ = _tmp78_->test_mode;
			if (_tmp79_) {
				GdkEventKey* _tmp80_ = NULL;
				GdkModifierType _tmp81_ = 0;
				_tmp80_ = event;
				_tmp81_ = _tmp80_->state;
				_tmp77_ = (_tmp81_ & GDK_MOD1_MASK) != 0;
			} else {
				_tmp77_ = FALSE;
			}
			if (_tmp77_) {
		...
		case GDK_KEY_Z:
		{
			...
			UnityGreeter* _tmp83_ = NULL;
			gboolean _tmp84_ = FALSE;
			_tmp83_ = unity_greeter_singleton;
			_tmp84_ = _tmp83_->test_mode;
			if (_tmp84_) {
				GdkEventKey* _tmp85_ = NULL;
				GdkModifierType _tmp86_ = 0;
				_tmp85_ = event;
				_tmp86_ = _tmp85_->state;
				_tmp82_ = (_tmp86_ & GDK_MOD1_MASK) != 0;
			} else {
				_tmp82_ = FALSE;
			}
			if (_tmp82_) {
	...
	_tmp87_ = event;
	_tmp88_ = GTK_WIDGET_CLASS (main_window_parent_class)->key_press_event ((GtkWidget*) G_TYPE_CHECK_INSTANCE_CAST (self, gtk_window_get_type (), GtkWindow), _tmp87_);
	result = _tmp88_;
	_g_object_unref0 (top);
	return result;
}
...
static void main_window_class_init (MainWindowClass * klass) {
	...
	((GtkWidgetClass *) klass)->size_allocate = main_window_real_size_allocate;
	((GtkWidgetClass *) klass)->motion_notify_event = main_window_real_motion_notify_event;
	((GtkWidgetClass *) klass)->key_press_event = main_window_real_key_press_event;
